...
def test_scrape_updates():
    html_content = pathlib.Path(
        "tests/assets/trybe_pages/novidades.html"
    ).read_text()
...
def test_scrape_next_page_link():
    first_page = pathlib.Path(
        "tests/assets/trybe_pages/novidades.html"
    ).read_text()

    second_page = pathlib.Path(
        "tests/assets/trybe_pages/novidades_2.html"
    ).read_text()
...
def test_scrape_news():
    sorted_news = []
    for new in sorted(os.listdir(NOTICIAS_PATH)):
